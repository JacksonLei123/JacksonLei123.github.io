...
function searchQuestion(event){
    ...
    if (event.keyCode == 13){
        ...
        if(categoryid.length > 0 || category.length == 0){
            ...
            xmlHttp.open("GET", "https://jservice.io/api/clues?min_date=" +date+  "&max_date=" +date + "&category=" + categoryID + "&value="+ difficulty, false );
            xmlHttp.send(null);
        ...
        for(j = 0; j < 1500; j++){
            ...
            categoryFinder.open("GET", "https://jservice.io/api/clues?min_date=" +date+  "&max_date=" +date + "&value="+ difficulty + "&offset="+j + "00" , false );
...
function generateRandomGameBoard(){
    ...
    while (replacementArray.length < 6) {
        var xmlHttp = new XMLHttpRequest();
        xmlHttp.open("GET", "https://jservice.io/api/random?count=100", false);
        xmlHttp.send(null);
        ...
        for(var i = 0; i< clues.length; i++){
            ...
            var xmlHttp = new XMLHttpRequest();
            xmlHttp.open("GET", "https://jservice.io/api/clues?category=" + clues[i].category_id + "&value=100", false);
            xmlHttp.send(null);
...
function clickTest(clickedId) {
  ...
  //  var category_id0 = JSON.parse(a.responseText)[clickedId[0]].category_id;
    ...
    var xmlHttp = new XMLHttpRequest();
    xmlHttp.open("GET", "https://jservice.io/api/clues?category=" + category_id0, false);
    xmlHttp.send(null);
    var categoryArray = JSON.parse(xmlHttp.responseText);
    // console.log(categoryArray);
